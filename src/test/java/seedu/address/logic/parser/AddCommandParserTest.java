...
public class AddCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        Student expectedStudent = new PersonBuilder(BOB).withTags(VALID_TAG_POOR).build();

        // whitespace only preamble
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + NAME_DESC_BOB + COURSE_DESC_BOB + EMAIL_DESC_BOB
                + TAG_DESC_POOR, new AddCommand(expectedStudent));


        // multiple tags - all accepted
        Student expectedStudentMultipleTags = new PersonBuilder(BOB).withTags(VALID_TAG_POOR, VALID_TAG_GOOD)
                .build();
        assertParseSuccess(parser,
                NAME_DESC_BOB + COURSE_DESC_BOB + EMAIL_DESC_BOB + TAG_DESC_GOOD + TAG_DESC_POOR,
                new AddCommand(expectedStudentMultipleTags));
    }

    @Test
    public void parse_repeatedNonTagValue_failure() {
        String validExpectedPersonString = NAME_DESC_BOB + COURSE_DESC_BOB + EMAIL_DESC_BOB
                + TAG_DESC_POOR;
        ...
        assertParseFailure(parser,
                validExpectedPersonString + COURSE_DESC_AMY + EMAIL_DESC_AMY + NAME_DESC_AMY
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, INVALID_NAME_DESC + COURSE_DESC_BOB + EMAIL_DESC_BOB
                + TAG_DESC_GOOD + TAG_DESC_POOR, Name.MESSAGE_CONSTRAINTS);

        // invalid course
        assertParseFailure(parser, NAME_DESC_BOB + INVALID_COURSE_DESC + EMAIL_DESC_BOB
                + TAG_DESC_GOOD + TAG_DESC_POOR, Course.MESSAGE_CONSTRAINTS);

        // invalid email
        assertParseFailure(parser, NAME_DESC_BOB + COURSE_DESC_BOB + INVALID_EMAIL_DESC
                + TAG_DESC_GOOD + TAG_DESC_POOR, Email.MESSAGE_CONSTRAINTS);

        // invalid tag
        assertParseFailure(parser, NAME_DESC_BOB + COURSE_DESC_BOB + EMAIL_DESC_BOB
                + INVALID_TAG_DESC + VALID_TAG_POOR, Tag.MESSAGE_CONSTRAINTS);
